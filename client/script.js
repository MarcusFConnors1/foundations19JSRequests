...
function helloBtnBlackGround() {
    sayHelloButton.style.backgroundColor = 'black';
    sayHelloButton.style.color = 'white';
}
sayHelloButton.addEventListener('mouseover', helloBtnBlackGround)
...
function helloBtnBlackGround2() {
    sayHelloButton.style.backgroundColor = '#EFEFEF'
    sayHelloButton.style.color = 'black'
}
sayHelloButton.addEventListener('mouseout', helloBtnBlackGround2)
...
const repeatMyParam = () => {
    //YOUR CODE HERE
    axios.get('http://localhost:3000/repeat/what')
    .then(res => {
...
function idk(){
    axios.get('http://localhost:3000/query-test/?myquery=name=idkwid&anotherOne=name=as;ldkfj;')
    .then((res) => console.log(res.data))
}
document.querySelector('#query-button').addEventListener('click', idk)
...
function createFood(event) {
    ...
    .then(res => {
        ...
        newq.textContent = res.data;
        bodyThing.appendChild(newq)
    })
}
const fromm = document.querySelector('form')
fromm.addEventListener('submit', createFood)